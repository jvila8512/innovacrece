        ...
        {tipoNoticiaList && tipoNoticiaList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="jhipsterApp.tipoNoticia.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {tipoNoticiaList.map((tipoNoticia, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/nomencladores/tipo-noticia/${tipoNoticia.id}`} color="link" size="sm">
                      {tipoNoticia.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button
                        tag={Link}
                        to={`/nomencladores/tipo-noticia/${tipoNoticia.id}`}
                        color="info"
                        size="sm"
